...
const PopUp = (props) => {
  ...
  const [todos, setTodos] = useState(
    JSON.parse(localStorage.getItem(`${props.card.id} - mmm`)) || []
  );
  ...
  const handleCommentsSubmit = (e) => {
    ...
    comments.map((comment) => {
      localStorage.setItem(`comment-${props.card.id}`, JSON.stringify(comments));
  ...
  const removeComment = (id) => {
    ...
    localStorage.setItem(`comment-${props.card.id}`, JSON.stringify(removedArr));
  };
  ...
  const addTodo = (todo) => {
    ...
    localStorage.setItem(`${props.card.id} - mmm`, JSON.stringify(newTodos));
    setTodos(newTodos);
  ...
  const removeTodo = (id) => {
    ...
    setTodos(removedArr);
    localStorage.setItem(`${props.card.id} - mmm`, JSON.stringify(removedArr));  };
          ...
          <div className="col-lg-7">
            ...
            <div className="w-100 border p-2 shadow-sm">
              <InputBase
                ...
                value={localStorage.getItem(`descrpiton-${props.card.id}`)}
                autoFocus
                onBlur={handleBlur}
            ...
            <div className="mt-4">
              <TodoList todos={todos} card={props.card} removeTodo={removeTodo} />
            </div>
